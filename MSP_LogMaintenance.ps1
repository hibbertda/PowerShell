...
[string]$EventSource = "MSP_LogMaintenance"
...
#endregion
#region Setup
Switch (Test-Path HKLM:\Software\MSP_Scripts\LogMaintenance) {
	$False {
		# Create script registry entries
		New-Item -Path HKLM:\Software\MSP_Scripts\LogMaintenance -Force
		New-ItemProperty -Path HKLM:\Software\MSP_Scripts\LogMaintenance -Name ScriptVersion -Value $ScriptVer
		New-ItemProperty -Path HKLM:\Software\MSP_Scripts\LogMaintenance -Name EventLogArchivePath -Value $EventLogArchivePath
		...
		schtasks /create /tn "MSP Event Log Archive" /xml $XMLExportPath
	...
	$True {	
		...
		$EventLogArchivePath = ((Get-ItemProperty -Path HKLM:\Software\MSP_Scripts\LogMaintenance).EventLogArchivePath)
		Write-Debug "EventLog Archive path: $EventLogArchivePath"
		[string]$EventLogArchiveTemp = $($EventLogArchivePath+"\_temp")
		Write-Debug "EventLog Archive temp path: $EventLogArchiveTemp"
        $message = "Starting MSP EventLog Archive Tool"
...
$message = "Stopping MSP EventLog Archive Tool"
